...
axiom(happens(shift_pack(Agnt,P,R1,R2,R3),T1,T6),
     ...
     before(T2,T3), not(clipped(T2,at(Agnt,R2),T3)), not(clipped(T1,in(P,R2),T3)),
     happens(pick_up(Agnt,P),T3), before(T3,T4), happens(go_to_room(Agnt,R2,R3),T4,T5),
     before(T5,T6), not(clipped(T3,got(Agnt,P),T6)), not(clipped(T5,at(Agnt,R3),T6)),
     happens(put_down(Agnt,P),T6)]).

axiom(initiates(shift_pack(Agnt,P,R1,R2,R3),in(P,R3),T),
     [holds_at(at(Agnt,R1),T), holds_at(in(P,R2),T)]).
...
axiom(initiates(go_to_room(Agnt,R1,R2),at(Agnt,R2),T),[holds_at(at(Agnt,R1),T)]).
...
axiom(initiates(pick_up(Agnt,P),got(Agnt,P),T),
     [diff(P,robot), holds_at(in(P,R),T), holds_at(at(Agnt,R),T)]).

axiom(releases(pick_up(Agnt,P),in(P,R),T),
     [diff(P,robot), holds_at(in(P,R),T), holds_at(at(Agnt,R),T)]).


axiom(initiates(put_down(Agnt,P),in(P,R),T),
     [diff(P,robot), holds_at(got(Agnt,P),T), holds_at(at(Agnt,R),T)]).

axiom(terminates(put_down(Agnt,P),got(Agnt,P),T),[]).


axiom(initiates(go_through(Agnt,D),at(Agnt,R1),T),
     [connects(D,R2,R1), holds_at(at(Agnt,R2),T)]).

axiom(terminates(go_through(Agnt,D),at(Agnt,R),T),[holds_at(at(Agnt,R),T)]).
...
axiom(initially(at(a1,r3)),[]).

axiom(initially(neg(at(a1,r1))),[]).

axiom(initially(neg(at(a1,r2))),[]).
