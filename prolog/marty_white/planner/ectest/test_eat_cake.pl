...
do_test(G) :- G= {happens(eat_cakes(1),now),holds_at(num_cakes(0),aft)}, solve_goal(G,R).

% rus out of stack but should just fail
do_test(G) :- G= {happens(eat_cakes(1),start),holds_at(num_cakes(1),aft)}, solve_goal(G,R).
