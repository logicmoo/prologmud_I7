...
golorpish:- only_pfc(nodebugx(golorp)).
...
get_session_id_local(O):- only_if_pfc(must(get_session_id(O))),!.

:- else.
get_session_id_local(O):- only_if_adv(must(current_input(O))),!.
:- thread_local(current_agent/1).
% current_agent(O):- must(current_input(O)),!.
:- endif.

foc_current_agent(P):- current_agent(P),!.
foc_current_agent(P):- gensym(agent,P),asserta(current_agent(P)),!.
...
unset_player_telnet_options(P):-
     ...
     only_pfc(clr(repl_writer(P,_))),
     only_pfc(clr(repl_to_string(P,_))).
...
set_local_modules(BaseKB,Undo):-
  ...
  Undo = ('$set_typein_module'(WasTM),'$set_source_module'(WasSM)),!.
...
session_loop(In,Out):-
  ...
  get_session_id_local(O),
  ensure_player_attached(In,Out,P),
  only_pfc(call_u(start_agent_action_thread)),
  only_pfc(ignore(look_brief(P))),!,
...
enqueue_session_action(A,L,S):- show_call(must(call_u(dbug(enqueue_agent_action(A,L,S))))),!.
...
telnet_repl_obj_to_string(O,_TypeHint,S):- only_if_pfc(must(object_string(O,S))),!.
...
look_brief(Agent):- only_if_pfc((prop(Agent,mudLastCommand,X),nonvar(X),functor(X,actLook,_))),!.

look_brief(Agent):- only_if_adv(call_u(look_as(Agent))),!.
look_brief(Agent):- !,only_if_pfc(call_u(look_as(Agent))),!.
