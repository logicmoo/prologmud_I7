...
cant(Agent, Action, cant(sense( Sense, Thing, Why)), State) :-
  never_equal(Sense,Thing),
...
cant(Agent, Action, cant(sense( Sense, Thing, Why)), State) :-
  ...
  act_verb_thing_model_sense(Action, Verb, Thing, _Spatial, Sense),
...
cant(_Agent, put(_Spatial, Thing1, _Prep, Thing1), self_relation( Thing1), _S0).
cant(_Agent, put(Spatial, Thing1, _Prep, Thing2), moibeus_relation( Thing1, Thing2), S0) :-
  related(Spatial, descended, Thing2, Thing1, S0).
cant(_Agent, throw( Thing1, _Prep, Thing1), self_relation( Thing1), _S0).
cant(_Agent, throw( Thing1, _Prep, Thing2), moibeus_relation( Thing1, Thing2), S0) :-
...
cant(Agent, examine(Sense, Thing), cant(sense( Sense, Thing, TooDark)), State) :- equals_efffectly(sense, Sense, see),
  never_equal(Sense,Thing),
  sensory_model_problem_solution(Sense, Spatial, TooDark, _EmittingLight),
  \+ has_sensory(Spatial, Sense, Agent, State).

cant(Agent, examine(Sense, Thing), cant(sense( Sense, Thing, Why)), State) :-
  never_equal(Sense,Thing),
  \+ can_sense( Sense, Thing, Agent, State),
  (Why = ( \+ can_sense( Sense, Thing, Agent, State))).


cant(Agent, goto(_Walk, _Dir, _Relation, Object), mustdrop(Object), State) :- nonvar(Object),
  related(_Spatial, descended, Object, Agent, State).

cant(Agent, EatCmd, cantdothat(EatCmd), State) :-
  getprop(Agent, can_do(EatCmd, f), State).
...
act_verb_thing_model_sense0(goto(_Walk, _Dir, _Rel, Thing), goto, Thing, spatial, see):-!.
