...
:- module(adv_server, [srv/0]).

:- dynamic(adv:wants_quit/4).
:- dynamic(adv:console_info/8).
...
adv_server(Port) :-
    ...
    thread_create(adv_server_loop(ServerSocket), _, 
                  [ alias(adv_server)
                  ]).

peer_alias(Peer, Host, Alias):- 
    tcp_host_to_address(Host, Peer), 
    (   Postfix = []
    ;   between(2, 1000, Num), 
        Postfix = [-, Num]
    ), 
    atomic_list_concat(['adv_client@', Host | Postfix], Alias), !.

adv_server_loop(ServerSocket) :-
   ...
   set_stream(OutStream, close_on_abort(true)), 
    ...
    ignore(catch(thread_create(
              adv_serve_client(InStream, OutStream, Slave, Host, Peer, Alias), 
              _, 
              [ alias(Alias)
              ]), 
...
adv_serve_client(InStream, OutStream, Slave, Host, Peer, Alias) :-    
   ...
   set_stream(user_output, close_on_abort(false)), 
    ...
    format(OutStream, 
           ...
           [adv_serve_client(Id,Alias,InStream,OutStream, Slave, Host, Peer)]), !, 
    call_cleanup(srv_catch(adventure_client_process(Id,Alias,InStream,OutStream, Slave, Host, Peer)), 
...
adventure_client_cleanp(Id,Alias,InStream,OutStream):- 
  srv_catch((adv:console_info(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent) -> 
     ((assertz(adv:agent_discon(Agent)),
       dmsg((adv:agent_discon(Agent))),
        stream_property(Err,file_no(2)),
        set_stream(Err,alias(Agent)),
       dmsg(adventure_client_cleanp_agent(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent)))) ;
      dmsg(failed_adventure_client_cleanp(Id,Alias,InStream,OutStream)))),
  retractall(adv:console_info(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent)),
...
guess_previous_agent(_, Peer, Agent):- adv:peer_agent(Peer, Agent),!.
guess_previous_agent(Host, _, Agent):- adv:peer_agent(Host, Agent),!.
guess_previous_agent(_Host, _Peer, Agent):- gensym(telnet,Agent).

prompt_for_agent(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent,Name):- 
  ...
  asserta(adv:console_info(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent)), 
  assertz(adv:agent_conn(Agent,Name,Alias,adventure_client_process(Id,Alias,InStream,OutStream, Slave, Host, Peer))),!.


adventure_client_process(Id,Alias,InStream,OutStream, Slave, Host, Peer):- 
  prompt_for_agent(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent,_Name),
...
adv_tlnet_readloop(Id,Alias):-    
   adv:console_info(Id,Alias,_InStream,_OutStream,_Slave,_Host,_Peer, Agent),
...
adv_tlnet_readloop(Id,Alias):-   
  ...
  % 
   ...
   ignore_srv_catch(adv_tlnet_words(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent, Words)).


adv_tlnet_words(_Id,_Alias,_InStream,_OutStream, _Slave, _Host, _Peer, _Agent, [prolog]):- !, prolog.

adv_tlnet_words(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent, [quit]):-
  nop(adv_tlnet_words(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent)),
  asserta(adv:wants_quit(Id,Alias,InStream,OutStream)).

adv_tlnet_words(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent, Words0):-
   nop(adv_tlnet_words(Id,Alias,InStream,OutStream, Slave, Host, Peer, Agent, Words0)),
